...
language_enum = sa.Enum("ru", "kg", "en", "tr", name="language")
study_form_enum = sa.Enum("full_time", "part_time", name="studyform")
currency_enum = sa.Enum("KGS", "USD", "EUR", name="currency")
document_type_enum = sa.Enum(
    "admission_rules", "fee_table", "deadlines", "other", name="documenttype"
)
tag_type_enum = sa.Enum("interest", "strength", "subject", "career", name="tagtype")
user_role_enum = sa.Enum("applicant", "admin", "superadmin", name="userrole")
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("study_form", study_form_enum, nullable=False),
    ...
    op.create_index(
        "ix_programs_university_lang_form_active",
        "programs",
        ["university_id", "language", "study_form", "is_active"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    study_form_enum.drop(op.get_bind(), checkfirst=True)
